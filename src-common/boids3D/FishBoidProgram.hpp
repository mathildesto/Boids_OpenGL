...
struct FishBoidProgram {
    ...
    void draw(std::vector<Boid3D>& boids, glm::mat4 projectionMatrix, glm::mat4 MVMatrix, ParamBoids3D& param,  Window3D& window, p6::Context&ctx, Camera::Freefly &freefly){    
        ...
        glUniform3fv(uLightDirVector, 1, glm::value_ptr(glm::vec3(freefly.getViewMatrix() * glm::vec4{1.f,1.f,1.f, 1.f}))); 
